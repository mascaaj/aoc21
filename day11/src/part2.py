...
class FlashFinder():
    ...
    def solve(self, start_i, start_j):
        # This has been located as a value >=10, set to zero
        self.matrix[start_i][start_j] = 0
        # This is a flash, increment count
        self.count += 1
        queue = collections.deque()
        # start with this value, move in x,y
        queue.append((start_i, start_j))
        while queue:
            (i, j) = queue.popleft()
            # Reset visited, this is a whole new iteration
            self.reset_visited()
            for move in range(len(self.move_x)):
                next_x = i + self.move_x[move]
                next_y = j + self.move_y[move]
                if self.is_valid(next_x, next_y):
                    # update visited for this iteration
                    self.visited[next_x, next_y] = 1
                    # Ensure all except start value is bumped
