...
def column_expansion(x, threshold):
    expanded = np.hstack([x, x+1, x+2, x+3, x+4])
    cleaned = np.where(expanded <= threshold, expanded, expanded - threshold)
    return cleaned


def row_expansion(x, threshold):
    expanded = np.vstack([x, x+1, x+2, x+3, x+4])
    cleaned = np.where(expanded <= threshold, expanded, expanded-threshold)
    return cleaned


def expand_matrix(x, threshold):
    return(row_expansion(column_expansion(x, threshold), threshold))
...
if __name__ == "__main__":
    ...
    input_map = expand_matrix(input_map.astype(int).reshape(lncount, lnwidth), 9)
